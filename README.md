# 🌟 내 인생의 버킷리스트 · 취미 & 세컨라이프

직접 해보고, 함께 웃고, 기억으로 남길 인생의 버킷리스트를 관리하는 개인 홈페이지입니다.
하나의 홈페이지에서 **두 가지 관점**의 버킷리스트를 상단 메뉴로 전환하며 관리합니다.

| 메뉴 | 설명 |
|---|---|
| 🎨 **Bucket-List for Hobby** (취미) | 바쁜 일상 속에서 놓치기 쉬운 가족·건강·자연·여행·배움·기록·나눔과 휴식 — 직접 해보고 기억으로 남길 작은 행복 300가지 |
| 🌅 **Bucket-List for 2nd Life** (세컨라이프) | 인생 2막에 이루고 싶은 귀농과 땅·영어·연구와 논문·오픈소스·지식 나눔과 유튜브·집필·여행·건강·가족·봉사 등의 목표 (<https://leemgs.github.io> 배경 자료 기반) |

**▶ 홈페이지: <https://leemgs.github.io/bucket-list/>**

- 취미 버킷리스트: <https://leemgs.github.io/bucket-list/?list=hobby&view=list>
- 세컨라이프 버킷리스트: <https://leemgs.github.io/bucket-list/?list=life2&view=list>
...
**🎨 취미(Hobby) 15개 분야**: 가족과 사랑 ❤️ · 농촌과 자연 🌾 · 여행과 세계 ✈️ · 건강과 몸 💪 ·
연구와 창작 🔬 · 나눔과 사회 🤝 · 배움과 도전 📚 · 기록과 추억 📷 · 휴식과 일상 행복 ☕ ·
은퇴와 인생 완성 🌅 · 사람과 관계 👥 · 소소한 모험 🧭 · 집과 농촌 프로젝트 🏡 · 문화와 창작 🎨 · 마음과 유산 🕊️

**🌅 세컨라이프(2nd Life) 12개 분야**: 귀농과 땅 🌱 · 영어와 언어 🗣️ · 연구와 논문 🔬 ·
오픈소스와 기술 💻 · 지식 나눔과 교육 📚 · 집필과 창작 ✍️ · 여행과 세계 ✈️ · 건강과 체력 💪 ·
가족과 관계 👨‍👩‍👧‍👦 · 마음과 유산 🕊️ · 자산과 은퇴 설계 💰 · 봉사와 커뮤니티 🤝
...
- **원본 데이터**: 취미는 `docs/data.json`, 세컨라이프는 `docs/data-2ndlife.json`
  (각 메뉴마다 별도의 파일에 커밋되어 서로 독립적으로 관리됩니다)
- **작업 사본**: 브라우저 localStorage (메뉴별로 분리된 키에 모든 변경이 즉시 저장됨)
- 페이지를 열면 원격 데이터 파일과 로컬 중 **더 최신인 쪽**을 자동으로 사용하고,
...
bucket-list/
├── README.md                      # 이 문서
├── my_life_bucket_list_300.html   # 최초 취미 버킷리스트 원본 (300개, 보관용)
└── docs/                          # GitHub Pages 사이트 루트
    ├── index.html                 # 버킷리스트 관리 홈페이지 (취미/세컨라이프 통합 단일 파일 앱)
    ├── data.json                  # 취미 버킷리스트 현재 데이터 (홈페이지가 자동 커밋)
    ├── data.js                    # 취미 초기 시드 데이터 (초기화 기능용)
    ├── data-2ndlife.json          # 세컨라이프 버킷리스트 현재 데이터 (홈페이지가 자동 커밋)
    └── data-2ndlife.js            # 세컨라이프 초기 74개 시드 데이터 (초기화 기능용)
```
